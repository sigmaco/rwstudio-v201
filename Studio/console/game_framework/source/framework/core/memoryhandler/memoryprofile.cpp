   ...
   namespace MemoryProfile
   {
      ...
      */
      void Open(TimeCallBackFunc _TimeCallBackFunc, RwBool StartEnabled)
      {
         ...
         outputbuffer = RWS_NEW char[OUTPUT_BUFFER_SIZE + 1];  // *TRUE ALLOC*

         bIsEnabled = StartEnabled;

         // Note, the RWS_NEW above is reported as the memory profiler can't profile until after this point.
